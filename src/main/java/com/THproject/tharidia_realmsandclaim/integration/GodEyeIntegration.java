package com.THproject.tharidia_realmsandclaim.integration;
...
public class GodEyeIntegration {
    ...
    private static void initialize() {
        ...
        try {
            ...
            if (godEyeDatabase != null) {
                ...
                initialized = true;
                TharidiaRealmsAndClaim.LOGGER.info("GodEye integration initialized successfully");
            } else {
                TharidiaRealmsAndClaim.LOGGER.warn("GodEye database not available yet");
            }
        } catch (ClassNotFoundException e) {
            TharidiaRealmsAndClaim.LOGGER.info("Tharidia Features mod not found, GodEye integration disabled");
            initializationFailed = true;
        } catch (Exception e) {
            TharidiaRealmsAndClaim.LOGGER.error("Failed to initialize GodEye integration", e);
            initializationFailed = true;
        }
    }
    ...
    public static void syncPlayerClaims(UUID playerUUID, ServerLevel level) {
        ...
        try {
            ...
            TharidiaRealmsAndClaim.LOGGER.debug("Synced {} claims to GodEye database for player {}", claims.size(), playerUUID);
        } catch (Exception e) {
            TharidiaRealmsAndClaim.LOGGER.error("Failed to sync player claims to GodEye database", e);
        }
    }
